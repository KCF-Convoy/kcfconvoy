...
class KCFmat(Library):
    ...
    def _append_kcf_vec(self):
        ...
        self.all_strs = list(set(self.all_strs) | kcf_vec.kcf_vec.keys())
    ...
    def calc_kcf_matrix(self, ratio=400):
        self.all_mat = np.array(
            [
                [self._get_num_string(kcfvec, string) for string in self.all_strs]
    ...
    def feature_selection(
        self, y, classifier, num_trial=10, corrcoef_threshold=0.8, max_features=2048
    ):
        X = self.mat
            ...
            if len(self.selected_features) == 0:
                ...
                for feature_idx2 in self.selected_features:
                    corrcoef = np.corrcoef(
                        self.mat.T[feature_idx], self.mat.T[feature_idx2]
                    )[0][1]
    ...
    def fps_mat(self):
        ret_list = [
            [int(fp.ToBitString()[i : i + 1]) for i in range(len(fp.ToBitString()))]
            for fp in self.fps
        ]
        return ret_list
    ...
    def draw_cpds(self, kcfstring="", kcfstringidx=False):
        if kcfstring == "":
            if kcfstringidx:
                ...
                return Draw.MolsToGridImage(
                    mols, molsPerRow=3, useSVG=True, legends=self.names
                )
        highlightAtomLists = []
        highlightmols = []
        highlightnames = []
        for name, kcfvec in zip(self.names, self.kcf_vecs):
            ...
            highlightAtomLists.append(tuple([int(n) for n in list1 if n != ""]))
